...
from langchain.memory import ConversationBufferMemory  # 聊天机器人核心模块，ConversationBufferMemory存储聊天机器人上下文对话

# 2.标题
st.title("黑马智聊机器人")

# 3.判断是否有历史数据记录对象，如果没有就创建，并存储所有记录消息
if 'memory' not in st.session_state:  # session_state 存储会话状态的子带你，用于存储会话数据
    #3.1创建一个ConversationBufferMemory对象，并且将数据存储到session_state
    st.session_state.memory=ConversationBufferMemory()
    #3.2添加机器人欢迎语句
    st.session_state.message=[{'role':'assistant','content':'你好，我是黑马智聊机器人，有什么可以帮助你的吗？'}]
    # 4.遍历session_state.message里面所有记录
for message in st.session_state.message:
    #4.1通过 聊天消息容器，用于显示当前角色内容
...
#6.判断用户录入的问题是否为空。不为空，我们才向下执行
if promt:
    #7.把用户信息添加到历史记录中，并且展示给前端
